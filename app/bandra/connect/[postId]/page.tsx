...
export default function PostDetailPage() {
  ...
  const structuredData = post ? {
    ...
    "url": `https://gharconnect.in/bandra/connect/${post.id}`,
    "publisher": {
      "@type": "Organization",
      "name": "GharConnect",
  ...
  return (
    <>
      <Head>
        <title>{post ? `${post.title} - Bandra Community Discussion | GharConnect` : 'Community Discussion - Bandra | GharConnect'}</title>
        <meta name="description" content={post ? `Join the discussion about "${post.title}" in the Bandra community. Read comments, share your thoughts, and connect with neighbors in Bandra.` : 'Join community discussions in Bandra. Share thoughts, ask questions, and connect with neighbors.'} />
        <meta name="keywords" content="Bandra community discussion, neighborhood forum, local discussions Bandra, community posts, Bandra residents forum" />
        <meta name="robots" content="index, follow" />
        <meta property="og:title" content={post ? `${post.title} - Bandra Community Discussion | GharConnect` : 'Community Discussion - Bandra | GharConnect'} />
        <meta property="og:description" content={post ? `Join the discussion about "${post.title}" in the Bandra community. Read comments, share your thoughts, and connect with neighbors in Bandra.` : 'Join community discussions in Bandra. Share thoughts, ask questions, and connect with neighbors.'} />
        <meta property="og:type" content="article" />
        <meta property="og:url" content={`https://gharconnect.in/bandra/connect/${postId}`} />
        <meta property="og:site_name" content="GharConnect" />
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:title" content={post ? `${post.title} - Bandra Community Discussion | GharConnect` : 'Community Discussion - Bandra | GharConnect'} />
        <meta name="twitter:description" content={post ? `Join the discussion about "${post.title}" in the Bandra community.` : 'Join community discussions in Bandra.'} />
        <link rel="canonical" href={`https://gharconnect.in/bandra/connect/${postId}`} />
        {structuredData && (
          <script
      ...
      <main className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-2xl mx-auto py-8 px-4">
          <nav aria-label="Breadcrumb">
            <Link href="/bandra/connect" className="text-blue-600 hover:underline mb-4 inline-block">&larr; Back to all posts</Link>
          </nav>
          {loading ? (
            <div>Loading...</div>
          ) : !post ? (
              ...
              <article className="bg-white p-4 rounded shadow mb-6">
                <header>
                  <div className="flex items-center gap-2 mb-2">
                    <span className="font-semibold text-sm text-gray-600">{post.category || 'gc/bandra'}</span>
                  </div>
