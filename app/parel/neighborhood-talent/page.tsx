...
export default function ParelNeighborhoodTalentPage() {
  ...
  useEffect(() => {
    filterTalents();
  }, [talents, searchTerm, selectedCategory]);
  ...
  const fetchTalents = async () => {
    try {
      ...
      const parelTalents = getProvidersBySociety('parel');
      setTalents(parelTalents);
    } catch (error) {
  ...
  const filterTalents = () => {
    ...
    // Apply search filter
    if (searchTerm) {
      filtered = searchProviders(searchTerm, 'parel');
    }

    // Apply category filter
    if (selectedCategory !== 'all') {
      filtered = getProvidersByCategory(selectedCategory, 'parel');
    }
      ...
      <div className="relative">
        <div className="w-full h-40 bg-indigo-600 flex items-center justify-center">
          <div className="text-center">
            <h1 className="text-3xl md:text-4xl font-bold text-white mb-2">🏠 Home Service Providers in Parel</h1>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-6">
            <p className="text-gray-600">
              {loading ? 'Loading talents...' : `Found ${filteredTalents.length} talented people in Parel`}
            </p>
          </div>
          ...
          {!loading && filteredTalents.length === 0 && (
            <div className="text-center py-12">
              ...
              <h3 className="text-xl font-semibold text-gray-900 mb-2">No talents found in Parel</h3>
