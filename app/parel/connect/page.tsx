...
export default function ParelConnectPage() {
  ...
  const [newPost, setNewPost] = useState({ title: '', body: '', category: 'gc/parel', images: [] as string[] });
  ...
  const fetchTopUsers = async () => {
    ...
    // Get posts count by user for Parel area
    const { data: postsData, error: postsError } = await supabase
      .from('posts')
      .select('user_id')
      .eq('area', 'Parel');
    
    // Get comments count by user for Parel posts
  ...
  const handleNewPost = async (e: React.FormEvent) => {
    ...
    setSubmitting(false);
    if (!error) {
      setNewPost({ title: '', body: '', category: 'gc/parel', images: [] });
      fetchPosts();
    ...
    <>
      ...
      <main className="min-h-screen bg-gray-50 lg:pt-16">
        ...
        <section className="w-full h-32 bg-indigo-600 flex flex-col items-center justify-center text-center mb-4">
          <h1 className="text-xl md:text-3xl font-bold text-white mb-1">👋 Welcome to Parel's Own Social Network</h1>
                    ...
                    <React.Fragment key={post.id}>
                      <li className="bg-white rounded-xl shadow border border-gray-100 p-3 md:p-5 flex flex-col gap-2">
                        ...
                        <div className="flex items-center gap-2 text-xs md:text-sm text-gray-500 mb-1">
                          ...
                          <span className="font-semibold text-gray-800 text-xs md:text-sm">{post.category || 'gc/parel'}</span>
                        ...
                        <div className="flex gap-2 md:gap-3 mt-2">
                          ...
                          <Link href={`/parel/connect/${post.id}`} className="flex items-center gap-1 px-2 md:px-4 py-1.5 md:py-2 rounded-full bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold text-xs md:text-sm">
                            {/* Speech bubble icon */}
              ...
              <ul className="space-y-3">
                {topLikedPosts.map(post => (
                  <li key={post.id} className="flex items-center justify-between">
                    <Link href={`/parel/connect/${post.id}`} className="text-blue-700 font-medium hover:underline line-clamp-1 flex-1">{post.title}</Link>
