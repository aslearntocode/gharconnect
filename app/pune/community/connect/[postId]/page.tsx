...
export default function PostDetailPage() {
  ...
  const structuredData = post ? {
    ...
    "url": `https://gharconnect.in/pune/connect/${post.id}`,
    "publisher": {
      "@type": "Organization",
      "name": "GharConnect",
  ...
  return (
    <>
      <Head>
        <title>{post ? `${post.title} - Pune Community Discussion | GharConnect` : 'Community Discussion - Pune | GharConnect'}</title>
        <meta name="description" content={post ? `Join the discussion about "${post.title}" in the Pune community. Read comments, share your thoughts, and connect with neighbors in Pune.` : 'Join community discussions in Pune. Share thoughts, ask questions, and connect with neighbors.'} />
        <meta name="keywords" content="Pune community discussion, neighborhood forum, local discussions Pune, community posts, Pune residents forum" />
        <meta name="robots" content="index, follow" />
        <meta property="og:title" content={post ? `${post.title} - Pune Community Discussion | GharConnect` : 'Community Discussion - Pune | GharConnect'} />
        <meta property="og:description" content={post ? `Join the discussion about "${post.title}" in the Pune community. Read comments, share your thoughts, and connect with neighbors in Pune.` : 'Join community discussions in Pune. Share thoughts, ask questions, and connect with neighbors.'} />
        <meta property="og:type" content="article" />
        <meta property="og:url" content={`https://gharconnect.in/pune/connect/${postId}`} />
        <meta property="og:site_name" content="GharConnect" />
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:title" content={post ? `${post.title} - Pune Community Discussion | GharConnect` : 'Community Discussion - Pune | GharConnect'} />
        <meta name="twitter:description" content={post ? `Join the discussion about "${post.title}" in the Pune community.` : 'Join community discussions in Pune.'} />
        <link rel="canonical" href={`https://gharconnect.in/pune/connect/${postId}`} />
        {structuredData && (
          <script
      ...
      <main className="min-h-screen bg-gray-50 lg:pt-16">
        ...
        <div className="max-w-2xl mx-auto py-8 px-4">
          <nav aria-label="Breadcrumb">
            <Link href="/pune/community/connect" className="text-blue-600 hover:underline mb-4 inline-block">&larr; Back to all posts</Link>
          </nav>
          {loading ? (
            <div>Loading...</div>
          ) : !post ? (
              ...
              <article className="bg-white p-4 rounded shadow mb-6">
                <header>
                  <div className="flex items-center gap-2 mb-2">
                    <span className="font-semibold text-sm text-gray-600">{post.category || 'gc/pune'}</span>
                  </div>
