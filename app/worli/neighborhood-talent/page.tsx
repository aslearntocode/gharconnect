...
export default function WorliNeighborhoodTalentPage() {
  ...
  const filterTalents = () => {
    ...
    // Apply search filter
    if (searchTerm) {
      filtered = searchProviders(searchTerm, 'worli');
    }

    // Apply category filter
    if (selectedCategory !== 'all') {
      filtered = getProvidersByCategory(selectedCategory, 'worli');
    }
      ...
      <div className="relative">
        <div className="w-full h-40 bg-indigo-600 flex items-center justify-center">
          <div className="text-center">
            <h1 className="text-3xl md:text-4xl font-bold text-white mb-2">🏠 Home Service Providers in Worli</h1>
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredTalents.map((talent) => (
                <Link href={`/worli/home-service-provider/${talent.id}`} key={talent.id} className="block group">
