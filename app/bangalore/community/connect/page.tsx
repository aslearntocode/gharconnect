...
export default function ParelConnectPage() {
  ...
  const [newPost, setNewPost] = useState({ title: '', body: '', category: 'general', area: 'Bangalore', images: [] as string[] });
  ...
  const fetchPosts = async () => {
    ...
    const bangaloreAreas = [
      ...
      'Seshadripuram', 'Shivajinagar', 'Fraser Town', 'Cox Town', 'Richmond Town',
      'Langford Town', 'Shanthinagar', 'Wilson Garden', 'Adugodi', 'Ejipura',
      'Agaram', 'Murugeshpalya', 'Bommanahalli', 'Hosur Road', 'Sarjapur',
      'Outer Ring Road', 'Old Airport Road', 'New Airport Road', 'Bannerghatta Road',
      'Kanakapura Road', 'Mysore Road', 'Tumkur Road', 'Hosur Road', 'Old Madras Road'
    ];
  ...
  const fetchTopUsers = async () => {
    ...
    // Get posts count by user for Bangalore areas only
    const { data: postsData, error: postsError } = await supabase
      .from('posts')
      .select('user_id')
      .in('area', bangaloreAreas);
    
    // Get Bangalore area post IDs first
    const { data: bangalorePostIds, error: postIdsError } = await supabase
      .from('posts')
      .select('id')
      .in('area', bangaloreAreas);
    
    // Get comments count by user for Bangalore area posts only
    const { data: commentsData, error: commentsError } = await supabase
      .from('comments')
      .select('user_id')
      .in('post_id', bangalorePostIds?.map(post => post.id) || []);
  ...
  const handleNewPost = async (e: React.FormEvent) => {
    ...
    setSubmitting(false);
    if (!error) {
      setNewPost({ title: '', body: '', category: 'general', area: 'Bangalore', images: [] });
      fetchPosts();
  ...
  const filteredPosts = posts.filter(post => {
    ...
    if (activeAreaFilter === 'Bangalore') {
      // Show only posts with category starting with gc/bangalore/
      matchesArea = post.category?.startsWith('gc/bangalore/') ?? false;
  ...
  return (
    <>
      <Head>
        <title>Community Discussions & Connect with Neighbors in Bangalore | GharConnect</title>
        <meta name="description" content="Join the Bangalore community discussion forum. Share thoughts, ask questions, and connect with neighbors. Find local recommendations, events, and community updates across Bangalore." />
        <meta name="keywords" content="Bangalore community, neighborhood discussions, local events Bangalore, community forum, connect with neighbors, Bangalore residents, local recommendations, Bangalore society discussions" />
        <meta name="robots" content="index, follow" />
        <meta property="og:title" content="Community Discussions & Connect with Neighbors in Bangalore | GharConnect" />
        <meta property="og:description" content="Join the Bangalore community discussion forum. Share thoughts, ask questions, and connect with neighbors. Find local recommendations, events, and community updates across Bangalore." />
        <meta property="og:type" content="website" />
        <meta property="og:url" content="https://gharconnect.in/bangalore/community/connect" />
        <meta property="og:site_name" content="GharConnect" />
        <meta property="og:image" content="https://gharconnect.in/GC_Logo.png" />
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:title" content="Community Discussions & Connect with Neighbors in Bangalore | GharConnect" />
        <meta name="twitter:description" content="Join the Bangalore community discussion forum. Share thoughts, ask questions, and connect with neighbors." />
        <meta name="twitter:image" content="https://gharconnect.in/GC_Logo.png" />
        <link rel="canonical" href="https://gharconnect.in/bangalore/community/connect" />
        <script
                ...
                <div className="grid grid-cols-2 gap-2 px-2">
                  {featuredPosts.slice(0, 4).map((post) => (
                    <div
                      ...
                      onClick={() => window.location.href = `/bangalore/community/connect/${post.id}`}
                    >
        ...
        <div className="max-w-7xl mx-auto py-8 px-2 lg:px-4 flex flex-col lg:flex-row gap-4 lg:gap-6">
                    ...
                    <React.Fragment key={post.id}>
                      <li className="bg-white rounded-xl shadow border border-gray-100 p-3 md:p-5 flex flex-col gap-2">
                        ...
                        <div className="flex items-center gap-2 text-sm md:text-sm text-gray-500 mb-1">
                          ...
                          <span className="font-semibold text-gray-800 text-sm md:text-sm">{post.category || 'gc/bangalore/general'}</span>
                        ...
                        {/* Title and body */}
                        <div className="cursor-pointer" onClick={() => window.location.href = `/bangalore/community/connect/${post.id}`}>
                        ...
                        {post.images && post.images.length > 0 && (
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 mb-2 cursor-pointer" onClick={() => window.location.href = `/bangalore/community/connect/${post.id}`}>
                        ...
                        <div className="flex gap-2 md:gap-3 mt-2">
                          ...
                          <Link href={`/bangalore/community/connect/${post.id}`} className="flex items-center gap-1 px-2 md:px-4 py-1.5 md:py-2 rounded-full bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold text-sm md:text-sm">
                            {/* Speech bubble icon */}
                      ...
                      {index === 0 && (
                        <li key="eggs-delivery-card" className="bg-gradient-to-r from-yellow-50 to-orange-50 border border-yellow-200 rounded-lg p-4">
                          <div className="flex items-center justify-between">
                            ...
                            <Link 
                              href="/bangalore/community/delivery/eggs" 
                      ...
                      {index === 2 && (
                        <li key="bangalore-rentals-card" className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-lg p-4">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
          ...
          <aside className="hidden lg:block w-72 flex-shrink-0 sticky top-24 self-start">
              ...
              <ul className="space-y-2">
                {topLikedPosts.map(post => (
                  <li key={post.id} className="flex items-center justify-between">
                    <Link href={`/bangalore/community/connect/${post.id}`} className="text-blue-700 font-medium hover:underline line-clamp-1 flex-1">{post.title}</Link>
            ...
            <div className="bg-white rounded-xl shadow border border-gray-100 p-3 mt-4">
              ...
              <Link 
                href="/bangalore/community/home-service-provider" 
