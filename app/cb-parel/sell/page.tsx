...
import { sellApartments } from '@/app/cb-parel/data/sellApartments';

export default function SellPage() {
  // Group apartments by type
  const apartmentsByType = sellApartments.reduce((acc, apt) => {
    if (!acc[apt.type]) {
      acc[apt.type] = [];
    }
    acc[apt.type].push(apt);
    return acc;
  }, {} as Record<string, typeof sellApartments>);
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {['T1', 'T2', 'T3', 'T4', 'T5', 'T6'].map((type) => {
              ...
              return (
                <Link href={`/cb-parel/sell/${type.toLowerCase()}`} key={type} className="block group">
                    ...
                    <div>
                      ...
                      {apartments.length > 0 && (
                        <p className="text-sm text-gray-500 mt-1">
                          Starting from ₹{Math.min(...apartments.map(apt => apt.price)).toLocaleString()}
                        </p>
                      )}
                    </div>
                  </div>
