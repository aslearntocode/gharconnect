...
export default function RentPage() {
  ...
  function ApartmentGallery({ contact_phone }: { contact_phone: string }) {
    ...
    useEffect(() => {
      getMediaUrlsForContactPhone(contact_phone).then(urls => {
        setMediaUrls(urls);
        setLoading(false);
      });
    }, [contact_phone]);
    ...
    if (loading) {
      return <div className="animate-pulse bg-gray-200 w-32 h-24 rounded-lg"></div>;
    }

    if (mediaUrls.length === 0) {
      return <div className="w-32 h-24 bg-gray-100 rounded-lg flex items-center justify-center text-gray-400 text-xs">No media</div>;
    }
      ...
      <div className="flex gap-2 overflow-x-auto">
        {mediaUrls.map((url, i) => (
          <ApartmentMedia
            ...
            onClick={() => setPhotoModal({ images: mediaUrls, idx: i })}
          />
        ))}
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-gray-50">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-center w-full h-full">
                        {apt.status === 'inactive' ? (
                          ...
                          <Link
                            href={`/mahalaxmi/apply-for-rent/${apt.id}`}
                ...
                <div key={apt.id}>
                  ...
                  {expandedMobileIdx === idx && (
                    <div className="bg-gray-50 rounded-b-xl px-4 py-4">
                      ...
                      <div className="mt-4">
                        {apt.status === 'inactive' ? (
                          <span className="w-full bg-gray-300 text-gray-600 px-3 py-2 rounded-lg font-semibold text-sm cursor-not-allowed select-none text-center block">
                            Already Rented
                          </span>
                        ) : (
                          <a 
                            ...
                            className="w-full bg-green-600 hover:bg-green-700 text-white px-3 py-2 rounded-lg font-semibold text-sm text-center block cursor-pointer"
                          >
                            Get Owner's Contact
                          </a>
                        )}
      ...
      {/* SEO Structured Data */}
      <SEOScript location="Mahalaxmi" type="rent" />
