...
export default function PowaiNeighborhoodServiceProviderPage() {
  ...
  const fetchTalents = async () => {
    try {
      ...
      const powaiTalents = getProvidersBySociety('powai');
      setTalents(powaiTalents);
    } catch (error) {
  ...
  const filterTalents = () => {
    ...
    if (searchTerm) {
      filtered = searchProviders(searchTerm, 'powai');
    }
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-6">
            <p className="text-gray-600">
              {loading ? 'Loading talents...' : `Found ${filteredTalents.length} talented people in Powai`}
            </p>
          </div>
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredTalents.map((talent) => (
                <Link href={`/powai/home-service-provider/${talent.id}`} key={talent.id} className="block group">
          ...
          {!loading && filteredTalents.length === 0 && (
            <div className="text-center py-12">
              ...
              <h3 className="text-xl font-semibold text-gray-900 mb-2">No talents found in Powai</h3>
