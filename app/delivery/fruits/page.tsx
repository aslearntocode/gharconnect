...
import { fruitProducts } from '@/app/data/delivery/fruits';
import { useState } from 'react';

// Group products by brand to show as vendors
const vendors = Array.from(new Set(fruitProducts.map(p => p.brand))).map(brand => ({
  name: brand,
  products: fruitProducts.filter(p => p.brand === brand),
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="relative">
        <div className="w-full h-32 bg-blue-600 flex items-center justify-center">
          <h1 className="text-3xl md:text-4xl font-bold text-white">Fruit Vendors</h1>
        </div>
        {/* Filter/Search Bar */}
        <div className="absolute left-1/2 transform -translate-x-1/2 -bottom-7 w-full max-w-2xl z-10">
          <div className="bg-white rounded-2xl shadow-lg flex items-center px-4 py-3 gap-2">
            ...
            <input
              type="text"
              placeholder="Search for fruit vendors..."
                  ...
                  <div className="border-t border-gray-100 p-4 bg-gray-50">
                    <div className="space-y-4">
                      {vendor.products.map((product) => (
                        <div key={product.id} className="border-b border-gray-200 pb-3 last:border-0 last:pb-0">
                          ...
                          <div className="space-y-1">
                            {product.sizes.map((size, idx) => (
                              <div key={idx} className="flex justify-between items-center text-sm">
                                ...
                                <span className="text-gray-900 font-medium">${size.price.toFixed(2)}/{size.unit}</span>
                              </div>
                            ))}
                          </div>
                        </div>
