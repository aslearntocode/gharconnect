// Test script to demonstrate price drop functionality
  ...
  try {
    ...
    const { data: product, error: createError } = await supabase
      ...
      .insert({
        ...
        area: 'parel',
        title: 'Test iPhone 12 Pro Max',
        description: 'Excellent condition iPhone 12 Pro Max for testing price drops',
        price: 45000, // Initial price: ₹45,000
    ...
    // Step 2: Update the price to trigger a price drop
    console.log('Updating price to create price drop...')
    const { data: updatedProduct, error: updateError } = await supabase
      .from('marketplace_products')
      .update({ price: 38000 }) // New price: ₹38,000 (₹7,000 drop)
      .eq('id', product.id)
      .select()
      .single()
    ...
    console.log(`New Price: ₹${updatedProduct.price.toLocaleString()}`)
